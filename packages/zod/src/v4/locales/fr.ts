    ...
    switch (issue.code) {
      ...
      case "invalid_union": {
        ...
        if (_issue.inclusive === false) {
          return "Entrée invalide : correspond à plusieurs options d'union (attendu exactement une)";
        }
        ...
        if (inst?._zod?.values) {
          ...
          return `Entrée invalide : attendu l'un de ${util.joinValues(expectedValues, ", ")}, mais reçu ${received}`;
        }
