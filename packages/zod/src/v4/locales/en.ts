    ...
    switch (issue.code) {
      ...
      case "invalid_union": {
        ...
        if (_issue.inclusive === false) {
          return "Invalid input: matched multiple union options (expected exactly one)";
        }
        ...
        if (inst?._zod?.values) {
          ...
          return `Invalid input: expected one of ${util.joinValues(expectedValues, ", ")}, but received ${received}`;
        }
