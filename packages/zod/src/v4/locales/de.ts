    ...
    switch (issue.code) {
      ...
      case "invalid_union": {
        ...
        if (_issue.inclusive === false) {
          return "Ungültige Eingabe: mehrere Union-Optionen erfüllt (genau eine erwartet)";
        }
        ...
        if (inst?._zod?.values) {
          ...
          return `Ungültige Eingabe: erwartet einen von ${util.joinValues(expectedValues, ", ")}, aber erhalten ${received}`;
        }
